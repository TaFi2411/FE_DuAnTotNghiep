                    ...
                    <div class="row">
                      <div class="col-md-8">
                        <div v-for="item in order.items" :key="item.id || item.productName" class="d-flex mb-3">
                          ...
                          <button v-if="order.statusName === 'COMPLETED'" class="btn btn-sm btn-outline-dark mt-2"
                            @click.stop="openReviewModal(item.id)">
                            Đánh giá sản phẩm
                          </button>
                        </div>
                      ...
                      <div class="col-md-4 d-flex flex-column justify-content-between">
                        ...
                        <div class="text-end mt-3">
                          <button v-if="order.statusName === 'PENDING'" class="btn btn-cancel me-2"
...
async function submitReview() {
  ...
  if (!reviewForm.value.description.trim()) {
    return Swal.fire("Thiếu thông tin", "Vui lòng nhập nội dung đánh giá!", "warning");
  }
  ...
  try {
    const payload = {
      ...
      accountId: accountId,
      orderDetailId: reviewForm.value.orderDetailId,
      images: reviewForm.value.images.map(img => ({
        path: img.path
      }))
    };
