         ...
         <div class="col-lg-9">
             ...
             <div v-else>
                ...
                <div v-else class="text-center py-5">
                   <h4 class="text-muted fw-light mb-3">Chưa có sản phẩm phù hợp</h4>
...
const handleLocalSort = () => {
    if (sortBy.value === 'price_asc') {
        ...
        // Mặc định ID lớn nhất lên đầu (Mới nhất)
...
const fetchProducts = async () => {
  ...
  try {
    ...
    // Sắp xếp lại dữ liệu sau khi lấy về
    handleLocalSort();
...
const setSort = (type) => { 
    sortBy.value = type;
    handleLocalSort(); // Sắp xếp ngay lập tức
};
...
/* Input giá */
.price-input-wrapper { display: flex; align-items: center; gap: 8px; margin-bottom: 15px; }
...
.btn-apply { width: 100%; background: #222; color: #fff; border: none; padding: 8px; font-weight: 600; font-size: 0.85rem; border-radius: 4px; transition: 0.3s; }
...
/* ================= CSS PHẦN CARD SẢN PHẨM (GIỮ NGUYÊN 100% CŨ) ================= */
/* !!! VÙNG KHÔNG CHỈNH SỬA !!! */
