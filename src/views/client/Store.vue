          ...
          <div class="sidebar-wrapper sticky-top">
            ...
            <div class="filter-section">
              ...
              <div class="price-presets mb-3">
                <div v-for="(preset, index) in pricePresets" :key="index" class="preset-tag"
                  :class="{ active: isPresetActive(preset) }" @click="selectPreset(preset)">
                  {{ preset.label }}
                </div>
              </div>
              ...
              <div class="range-slider-container mb-4">
                ...
                <input type="range" min="0" :max="sliderLimit" step="100000" v-model.number="tempPrice.min"
                  @input="validateRange('min')" class="range-input min-range">

                <input type="range" min="0" :max="sliderLimit" step="100000" v-model.number="tempPrice.max"
                  @input="validateRange('max')" class="range-input max-range">
              </div>

              <div class="price-input-wrapper">
                <div class="input-group-tdt">
                  <span class="currency">₫</span>
                  <input type="text" :value="formatInput(tempPrice.min)" @input="e => updatePriceInput(e, 'min')"
                    @keyup.enter="applyFilters" placeholder="Từ">
                </div>
                <span class="range-divider">-</span>
                <div class="input-group-tdt">
                  <span class="currency">₫</span>
                  <input type="text" :value="formatInput(tempPrice.max)" @input="e => updatePriceInput(e, 'max')"
                    @keyup.enter="applyFilters" placeholder="Đến">
                </div>
              </div>
            ...
            <div class="d-flex align-items-center gap-3 mt-4 mt-md-0">
              ...
              <div class="dropdown">
                ...
                <ul class="dropdown-menu dropdown-menu-end border-0 shadow-lg mt-2 rounded-0">
                  ...
                  <li><a class="dropdown-item py-2" href="#" @click.prevent="setSort('price_desc')">Giá giảm dần</a>
                  </li>
                </ul>
              </div>
            </div>
                ...
                <div v-for="product in filteredProducts" :key="product.id" class="product-item">
                  <div class="card product-card h-100 text-center shadow-sm" @click="goToDetail(product.id)">
                    ...
                    <div class="card-body d-flex flex-column px-3 pb-3 pt-0">
                      <h6 class="product-name text-dark" :title="product.name">{{ product.name }}</h6>
...
.input-group-tdt input:focus {
  border-color: #000;
}

.input-group-tdt .currency {
  position: absolute;
  left: 8px;
  top: 50%;
  transform: translateY(-50%);
...
.pagination-tdt li.active button {
  color: #000;
  border-bottom: 2px solid #000;
}

.pagination-tdt li.disabled button {
  opacity: 0.5;
  cursor: not-allowed;
}

.pagination-tdt li:not(.active):not(.disabled) button:hover {
  color: #000;
}
</style>